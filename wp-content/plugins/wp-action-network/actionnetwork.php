	...
	if ($installed_version != $actionnetwork_version) {
		...
		if ($actionnetwork_version == '1.2.1') {
			$notices[] = sprintf(
				...
				__('The %s has been updated to version 1.2.1. This update provides a Gutenberg Editor button as well as a bug fix related to hCaptcha.', 'actionnetwork'),
...
function update_cron_log($inserted,$updated,$new_only){
	global $wpdb;
	$table_name = $wpdb->prefix . 'actionnetwork_cron_log';
	$wpdb->insert( 
		$table_name, 
...
function actionnetwork_process_queue(){
	...
	$cron_func = isset($_REQUEST['cron_func']) ? $_REQUEST['cron_func'] : 0;
	...
	if($cron_func==1){
		update_cron_log($inserted,$updated,$new_only);
	}
...
function _actionnetwork_admin_handle_actions(){
	...
	switch ($_REQUEST['actionnetwork_admin_action']) {
		...
		case 'update_spam_keys':
			update_option('actionnetwork_hcaptcha_site_key', $_REQUEST['actionnetwork_hcaptcha_site_key']);
			update_option('actionnetwork_hcaptcha_secret_key', $_REQUEST['actionnetwork_hcaptcha_secret_key']);
		break;
			...
			if ($queue_status != 'empty') {
				...
				if ($actionnetwork_api_key_is_valid) {
					...
					if ($actionnetwork_api_key) {
						...
						$args = array( 'body' => $body, 'timeout' => 30 );
                        //echo "POST RESPONSE:\n";
						$response = wp_remote_post( $ajax_url, $args );
                        //print_r($response);*/
                        
                        $_REQUEST['action'] = 'actionnetwork_process_queue';
                        $_REQUEST['queue_action'] = 'init';
		...
		if ($embed_wp_id) {
			...
			<h2 class="nav-tab-wrapper">
                ...
                <a href="$admin_url#actionnetwork-cron-updates">Cron Updates</a>
			</h2>
	...
	<div class='wrap'>
		...
		<div class="wrap-inner">
			...
			<h2 class="nav-tab-wrapper">
                ...
                <a href="#actionnetwork-cron-updates" class="nav-tab<?php echo ($tab == 'cron-updates') ? ' nav-tab-active' : ''; ?>">Cron Updates</a>
                
				<a href="#actionnetwork-about" class="nav-tab<?php echo ($tab == 'about') ? ' nav-tab-active' : ''; ?>">
					<?php _e('About', 'actionnetwork'); ?>
				</a>
			</h2>
			...
			<div class="actionnetwork-admin-tab<?php echo ($tab == 'cron-updates') ? ' actionnetwork-admin-tab-active' : ''; ?>" id="actionnetwork-cron-updates">
                    <h2>Cron Updates</h2>
					<?php global $wpdb;
					...
					<table cellpadding="10" border="1">
						...
						<tr>
							<th><label for="gender"><?php echo $row->id ?></label></th>
							<td><?php echo $row->time ?></td>
							<td><?php echo $row->inserted ?></td>
							<td><?php echo $row->updated ?></td>
							<td><?php echo $row->new_only ?></td>
						</tr>
						<?php } ?>					
					</table>
					<?php
			...
			<div class="actionnetwork-admin-tab<?php echo ($tab == 'settings') ? ' actionnetwork-admin-tab-active' : ''; ?>" id="actionnetwork-settings">
				...
				<h2><?php _e('Plugin Captcha Settings', 'actionnetwork'); ?></h2>
				<p>In order to be able to activate anti-spam challenges you have to fill up the forms bellow. In order to get the keys all you have to do is create
				an account on <a href="https://hcaptcha.com" target="_blank">hcaptcha.com</a>.</p>
				<form method="post" action="">
					<?php
					...
					<table class="form-table"><tbody>
						<tr valign="top">
							...
							<td>
								<input id="actionnetwork_hcaptcha_site_key" style="min-width: 400px" name="actionnetwork_hcaptcha_site_key" type="text" value="<?php esc_attr_e($hcaptcha_site_key); ?>" />
							</td>
						</tr>
						<tr valign="top">
							<th scope="row"><label for="actionnetwork_hcaptcha_secret_key"><?php _e('hCaptcha Secret Key', 'actionnetwork'); ?></label></th>
							<td>
								<input id="actionnetwork_hcaptcha_secret_key" style="min-width: 400px" name="actionnetwork_hcaptcha_secret_key" type="text" value="<?php esc_attr_e($hcaptcha_secret_key); ?>" />
							</td>
						</tr>
					</tbody></table>
					<p class="submit"><input type="submit" id="actionnetwork-options-form-submit" class="button-primary" value="<?php _e('Save Settings', 'actionnetwork'); ?>" /></p>
			...
			<div class="actionnetwork-admin-tab<?php echo ($tab == 'about') ? ' actionnetwork-admin-tab-active' : ''; ?>" id="actionnetwork-about">
				<h2><?php _e('About the Plugin', 'actionnetwork'); ?></h2>
								<style type="text/css">
...
function wdm_add_mce_button() {
    ...
    $script_decode = json_encode( $results_array );
...
EOF;
    ...
    if ( 'true' == get_user_option( 'rich_editing' ) ) {
        add_filter( 'mce_external_plugins', 'wdm_add_tinymce_plugin', 22, 1 );
...
add_action( 'wp_ajax_nopriv_getActionNetworks', 'get_action_networks' );
add_action( 'wp_ajax_getActionNetworks', 'get_action_networks' );

if ( ! function_exists( 'get_action_networks' ) ) {
	function get_action_networks() {
		global $wpdb;
