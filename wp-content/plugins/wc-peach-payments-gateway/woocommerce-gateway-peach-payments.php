...
function woocommerce_gateway_peach() {
	...
	class WC_Peach_Payments extends WC_Payment_Gateway {
		...
		?>
			...
			//API Rollback Code kept for reference
			/*
			$url = 'https://api.wordpress.org/plugins/info/1.0/wc-peach-payments-gateway.json';
			$rollback_versions = '';
			$response = wp_remote_get( $url );
			if ( wp_remote_retrieve_response_code( $response ) !== 200 ) {
				$btn_txt = '<p>Version rollback not available at the moment. Please try again in a few minutes.</p>';
			}else{
				$rollback_versions = json_decode($response['body']);
			}
			*/
			?>
		...
		public function payment_fields() {
			...
			/*
			//disabled on version 3.1.8
			if(empty($this->checkout_methods)){
				$logger_info['settings'] = $this->logger_info_settings;
				$logger_info['errors'] = 'No Payment Methods found.';
			}
			*/
		...
		public function process_payment( $order_id ) {
			...
			if(isset($_GET['id']) && isset($_GET['resourcePath'])){
				...
				if($auth_bearer == $this->accesstoken){
					...
					//echo '<pre>'.print_r(json_decode($responseData), true).'</pre>'; die();
				...
				if(isset($_POST['result_code'])){
					...
					if($_POST['result_code'] == $this->success_code){
						...
						//wp_safe_redirect( $this->order_received_page_url.'/?key='.$orderNew->get_order_key() );
		...
		function receipt_page( $order_id ) {
			...
			if($subscribe_test[1]){
				...
				if($payOption == 'other'){
					...
					wp_enqueue_script('peachpaymentSwitchPaymentJs',WC_PEACH_PLUGIN_URL.'/assets/js/switch_payment.js');
					...
					if(isset($responseData->id)){
						...
						$brands_check = array('VISA','MASTER','AMEX','DINERS');
						$brands = '';
						if(isset($this->consolidated_label_logos) && !empty($this->consolidated_label_logos)){
							foreach($this->consolidated_label_logos as $brand){
								if($payOption != 'other'){
									if(in_array($brand,$brands_check)){
										$brands .= $brand.' ';	
									}
								}else{
									$brands .= $brand.' ';
		...
		public function switch_payment_webhook_response() {
			...
			if ( false !== $order &&  $order_id != '') {
				...
				$peachpayment_error_message = 'Could not retrive order information.';
			}
		...
		public function pp_handle_switch_request( $data ) {
			...
			if ( $peachpayment_error ) {
				...
				//$resultCode =  esc_html($data['result.code']);
				if ($resultCode == $this->success_code ) {
			...
			if ( $peachpayment_error ) {
				...
				if($status != $this->peach_order_status || $status != 'completed' || $status != 'on-hold' || $status != 'refunded'){
					if ($resultCode == $this->success_code) {
						...
						//$order->update_status('pending', __( 'Peach Switch Webhook ['.$data['result_code'].']:'.woocommerce_clean($data['result_description']).'. ', 'woocommerce' ));
					}else{
						$logger_info['errors'] = array(
							'Step' => '3/4',
							'OrderID' => $seqOrderID,
...
function peach_required_scripts() { 
	...
	wp_localize_script('peach_front_js', 'peach_ajax_object', array( 'ajax_url' => admin_url( 'admin-ajax.php' ) ) );
...
function peach_display_order_data_in_admin( $order ){  ?>
	...
	if ( isset( $available_gateways['peach-payments'] ) ) {
		...
		$value = get_post_meta( $order->id, '_billing_peach', true );
...
function peach_cards_content() {
	...
	/*
	$newCard = array(
		'payment_id' => '8ac7a4a2886aa10c01886caaeb822e03',
		'active_card' => '1111',
		'brand' => 'VISA',
		'exp_year' => '2025',
		'exp_month' => '01'
	...
	update_user_meta( get_current_user_id(), '_peach_payment_id', $newCard);
	*/
...
function peach_add_card_content() {
	...
	if(isset($_GET['id']) && isset($_GET['resourcePath'])){
		...
		if(isset($responseData->id)){
			...
			$brands_check = array('VISA','MASTER','AMEX','DINERS');
			$brands = '';
			if(isset($consolidated_label_logos) && !empty($consolidated_label_logos)){
				foreach($consolidated_label_logos as $brand){
					if(in_array($brand,$brands_check)){
						$brands .= $brand.' ';	
					}
				}
...
function generateCards($action, $myCards, $myOldCards, $cardRemove){
	...
	if(is_array($myCards) && count($myCards) > 0){
		foreach($myCards as $card){
			//$cardInfo = peachRequestCardAPI($card['id']);
